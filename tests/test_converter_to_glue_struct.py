...
def test_json_schema_to_glue_struct_full_schema_different_jsonSchema():
    local_path = pathlib.Path(__file__).parent.absolute()
    schema = open(r'{}/examples/schemas/teste_schema_2.json'.format(local_path),)
    schemaLoad = json.load(schema)

    test = GlueStructTransform.json_schema_to_glue_struct(schemaLoad)

    resultAwaited = "campo1:double,campo2:double,campo3:string,campo4:boolean,campo5:struct<campo6:string,campo7:struct<campo8:double,campo9:array<struct<campo10:string,campo11:array<struct<campo12:boolean>>>>>>,campo13:double,campo14:array<struct<campo15:struct<campo16:string,campo17:boolean>>>"
